...
//	}
	...
	// FIX
	// ChatGPT suggeriva di usare questa funzione, in realtà lo stesso controllo è fatto in "Tipologia.java"
	private boolean isValidWhereClause(String where) {
	    String[] validWhereClauses = {"Gadget", "Action Figures", "Arredamento Casa"};
	    for (String valid : validWhereClauses) {
	        if (valid.equals(where)) {
	            return true;
	        }
	    }
	    return false;
	...
	public synchronized Collection<ProductBean> doRetrieveAll(String where) throws SQLException {
	    ...
	    // Validazione dell'input per evitare SQL injection
	    if (!isValidWhereClause(where)) {
	        throw new SQLException("Invalid where clause.");
	    }
